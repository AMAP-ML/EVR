...
# Per-image progress bar shared with the training loop; the trainer sets the
# total, reward calls running in executor threads advance it per image.
_reward_pbar = None


def set_reward_pbar(total):
    global _reward_pbar
    if _reward_pbar is not None:
        _reward_pbar.close()
    _reward_pbar = tqdm(total=total, desc="Waiting for rewards", position=0, dynamic_ncols=True)
...
def mllm_score_continue(device):
    ...
    batch_size = 1
    url = f"http://{os.getenv('REWARD_SERVER', 'localhost:12343')}/mode/logits_non_cot"
    ...
    def _fn(ref_images, images, prompts, metadatas):
        ...
        if not isinstance(ref_images, torch.Tensor):
            target_images = [pair[1] for pair in ref_images]
            ref_images = [pair[0] for pair in ref_images]
            ...
            target_images = np.array([np.array(img) for img in target_images])
            target_images_batched = np.array_split(target_images, np.ceil(len(target_images) / batch_size))
            
        all_scores = []
        for image_batch, ref_image_batch,target_image_batch in zip(images_batched, ref_images_batched, target_images_batched):
            ...
            target_jpeg_images = []
            for target_image in target_image_batch:
                img = Image.fromarray(target_image)
                buffer = BytesIO()
                img.save(buffer, format="JPEG")
                target_jpeg_images.append(buffer.getvalue())
            # format for LLaVA server
            data = {
                "ref_images": ref_jpeg_images,
                "sce_images": target_jpeg_images,
                "images": jpeg_images,
                "prompts": prompts,
                "metadatas": metadatas,
            }
            ...
            response = sess.post(url, data=data_bytes, timeout=1800)
            response_data = pickle.loads(response.content)

            all_scores += response_data["scores"]
            if _reward_pbar is not None:
                _reward_pbar.update(len(image_batch))
...
def unifiedreward_score_sglang(device):
    ...
    client = AsyncOpenAI(
        base_url=os.getenv("VLM_URL", "http://127.0.0.1:8000/v1"),
