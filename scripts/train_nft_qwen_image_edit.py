...
class PromptImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_image_path = self.metadatas[idx]["target_image"]
        item["prompt_with_image_path"] = f"{self.prompts[idx]}_{ref_image_path}"
        ref_image = Image.open(os.path.join(self.dataset, ref_image_path)).convert("RGB")
        target_image = Image.open(os.path.join(self.dataset, target_image_path)).convert("RGB")
        # image = Image.open(os.path.join(self.dataset, refimage_path)).convert("RGB")
    ...
    def collate_fn(examples):
        ...
        images = [[example["ref_image"],example["target_image"]] for example in examples]
...
class DistributedKRepeatSampler(Sampler):
    ...
    def __iter__(self):
        while True:
            ...
            repeated_indices = [idx for idx in indices for _ in range(self.num_replicas)]
...
def eval_fn(
    ...
    for test_batch in tqdm(
        ...
        # print(f"image::::{len(images)};{images[0].shape}")
        # print(f"ref_image::::{ref_images};{ref_images[0].shape}")
        rewards_future = executor.submit(
            reward_fn, images, prompts*len(images), prompt_metadata, ref_images*len(images), only_strict=False
        )
        time.sleep(0)
...
def main(_):
    ...
    if config.resume_from:
        ...
        if os.path.exists(lora_path):  # Check if it's a PEFT model save
            ...
            print(f'=========================================LORA RESUME FROM {config.resume_from}=============================================')
        ...
        if os.path.exists(opt_path):
            ...
            print(f'=========================================OPTIMIZER RESUME FROM {config.resume_from}=============================================')
        ...
        try:
            ...
            target_epoch = int(os.path.basename(config.resume_from).split("-")[-1])
            logger.info(
    ...
    for epoch in range(target_epoch, config.num_epochs):
        ...
        pipeline.transformer.eval()
        samples_data_list = []
        # per-image reward progress: num_batches * images per batch (train_batch_size)
        flow_grpo.rewards.set_reward_pbar(
            config.sample.num_batches_per_epoch * config.train.batch_size
        )
        ...
        for i in tqdm(
            ...
            rewards_future = executor.submit(
                reward_fn,
                images,
                prompts*config.train.batch_size,
                prompt_metadata,
                ref_images*config.train.batch_size,
                only_strict=True,
            )
            time.sleep(0)
            # import ipdb
            # ipdb.set_trace()
            timesteps = timesteps.repeat(config.train.batch_size,1)
            samples_data_list.append(
                {
                    "prompt_ids": prompt_ids.repeat(config.train.batch_size,1),
        ...
        if epoch % 10 == 0 and is_main_process(rank):
            ...
            prompts_to_log = prompts*config.train.batch_size  # from last sampling batch on this rank
